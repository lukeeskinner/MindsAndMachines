...
- **Login artwork:** a lazy-loaded, matte Three.js sculpture adds teal/sky depth beside the sign-in panel. Cognito branding appears beside the configured account sign-in action, never as the app brand or a claim about local demo authentication. The artwork moves automatically, respects reduced motion, pauses offscreen/in background tabs, disposes its GPU resources on exit, and retains a static fallback without WebGL. Dashboard styling and study interactions are unchanged.
